...
| 系统推送通知（新订单像手机消息一样弹出提醒） | `VAPID_PUBLIC_KEY`、`VAPID_PRIVATE_KEY`、`VAPID_SUBJECT`（可选） | 已生成好一组密钥，见下方"推送通知"一节 |
...
**已生成好的密钥**（已经在用了）：
```
VAPID_PUBLIC_KEY=BBhUXgsgwVnl9cAM_QyuTxD8puO1RPFwExzDZmqiJLVnBHBpW6S5HGvpNnqQx8pSNABOku5HQGX4ibceWqYoego
VAPID_PRIVATE_KEY=qoQBwZ4Ea4xjeftqJmmC0_fAm7TwMNrshQ26gBEsSOc
```
